...
txt_labels = {}
VERSE_AS_CHAPTER_EXCEPTIONS = {594}
...
# 2) batch 병합
chapters = {}
for bn in ['batch0_1_12','batch1_13_16','batch2_17_20','batch3_21_25','batch4_26_29','batch5_30_33','batch6_34_36']:
...
for ch in sorted(chapters):
    ...
    note = ('audited: MSG 2Chronicles 36장 전수 대조 (unit 배지 1:1). '
            '1-12장은 MSG 기준 신규 작성, 13-36장은 재청킹 초벌 + 의미 재감사. '
            '슬랭 6 rule 적용. merge/split은 적용하지 않음.')
    en_list.append({'chapter': ch, 'title': C.get('title_en',''), 'review_status': 'msg_audited',
                    'note': note, 'paragraphs': paras_en, 'verseRanges': vrs2,
                    'msg_ranges': msg_ranges, 'merges': C.get('merges', []), 'splits': C.get('splits', []), 'confirmations_needed': C.get('confirmations_needed', []),
                    'changes': C.get('changes', [])})
    ko_list.append({'chapter': ch, 'title': C.get('title_ko',''), 'review_status': 'msg_audited',
