...
⚠ 정정 (2026-09-24 코디네이터): 감사 워커가 fixes JSON에 § 헤더 7건(EN/KO 각)을 추가했으나, 27권 감사 컨벤션(헤더 제외, 프로덕션 시드 구조 유지)과 어긋나 제거함. 소제목은 빌더가 MSG 원문에서 독에 삽입. 제거 후 validator PASS·build 20행/VERIFY_DROPPED=0·pairing OK·독 재업로드+export-back VERIFIED.
...
### EN (본문 18건 — § 헤더 7건은 27권 컨벤션에 따라 감사 후 제거)
